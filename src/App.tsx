...
interface BuildingRowProps {
  ...
  setImagePopup: (v: { x: number; y: number; url: string; name: string } | null) => void;
...
export default function App() {
  ...
  const handleImportAll = async (parsed: BookmarkletData, targetProfileId?: string) => {
    ...
    const alliesFromGame = Allies.parseAllyData(parsed.allies, RARITY_FROM_GAME);
                  ...
                  <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div className="flex flex-col rounded-lg bg-slate-950 border border-violet-500/20">
                      <span className="px-3 pt-2 pb-1 text-xs font-bold text-violet-400">
                        {t("debugSelectionKits", uiLang)} ({allSelectionKits.length})
                      </span>
                      <div className="max-h-60 overflow-y-auto px-3 pb-2 text-xs font-mono text-slate-300">
                        {(() => {
                    ...
                    <div className="flex flex-col rounded-lg bg-slate-950 border border-sky-500/20">
                      <span className="px-3 pt-2 pb-1 text-xs font-bold text-sky-400">
                        {t("debugUpgradeKits", uiLang)} ({inventoryUpgradeKits.size})
                      </span>
                      <div className="max-h-60 overflow-y-auto px-3 pb-2 text-xs font-mono text-slate-300">
                        {(() => {
                        ...
                        >
                          <td className="cell-name">
                            ...
                            {a.isFragment ? (
                              ...
                              <span className="ml-1.5 inline-block text-[10px] font-mono font-bold px-1 py-0.5 rounded bg-emerald-950 text-emerald-400 border border-emerald-900" title={t("allyPlacedTitle", uiLang)}>🏠</span>
                            ) : (
